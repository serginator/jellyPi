...
This reads the API keys already in `.env`/generated by Sonarr/Radarr/Prowlarr and writes `services.yaml`, `widgets.yaml` and `settings.yaml` into `$STORAGE/config/homepage`. Jellyfin and Seerr are added as plain links by default (their API keys aren't auto-generated) — to get widgets for them too, create a key in **Jellyfin → Dashboard → API Keys** and **Seerr → Settings → General**, add them to `.env` as `JELLYFIN_API_KEY`/`SEERR_API_KEY`, then re-run:
